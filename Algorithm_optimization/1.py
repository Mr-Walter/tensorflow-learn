...
for i in range(len(query_picture)):
    Euclidean_distance=np.mean((img_feature-img_feature[query_picture[i]])**2,1) # [567297,]
    comp1_cls[:, i] = Euclidean_distance
    # comp1_cls[:, i+1]=distance(img_feature2,img_feature2[query_picture[i]])
    comp1_cls[:, i + 1] =np.mean((img_feature2 - img_feature2[query_picture[0]]) ** 2, 1,np.float16)
    # comp1_cls[:, i + 1]=np.mean(abs(img_feature2-img_feature2[query_picture[0]]),1,np.float32)
    # comp1_cls[:, i + 1] =np.sum(abs(img_feature2-img_feature2[query_picture[0]]), 1, np.float32)
    # comp1_cls[:, i + 1] = np.std((img_feature2 - img_feature2[query_picture[0]]), 1, np.float32)
    # err=img_feature2-img_feature2[query_picture[0]]
    # mean_err=np.mean(err,1)
    # comp1_cls[:, i + 1]=np.mean((err-mean_err[:,np.newaxis])**2,1,np.float32)
    #
