...
if __name__ == '__main__':
    ...
    if train == 0 or train == -1:
        ...
        # 连接一个隐藏层作为特征编码
        net = tf.layers.dense(feature, 4096, activation=tf.nn.relu,trainable=flag, # 刚开始 trainable=True 训练一定程度后设置为False
                              name='fc2')  # [n,4096]
        net = tf.layers.dense(net, 32, activation=tf.nn.sigmoid, # 32,16,8  # 因为其他层都设置为trainable=False ，则只会更新编码层(降低lr 微调该层)
                              name='fc3')  # [n,48]  # Binary code layer
        # net=tf.round(net) # 第一次训练不加，第二次加上

        pred = tf.layers.dense(net, num_classes, activation=tf.nn.softmax,name='output2') # 这一层不能设置trainable=False
    ...
    index = np.arange(0, mnist.train.num_examples)
