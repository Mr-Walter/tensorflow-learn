...
def entropy(ivf_vecs):  # ivf_vecs shape [batch_size,k]
    ...
    ivf_vecs += zero;
    entropy = -tf.reduce_sum(tf.multiply(ivf_vecs, tf.log(ivf_vecs))) / (
                tf.cast(tf.shape(ivf_vecs)[0], tf.float32) * tf.log(tf.cast(2, tf.float32)))
    return entropy
...
if __name__ == "__main__":
    ...
    with tf.Session(config=config) as sess:
        ...
        if train == 0:
            ...
            if (
            not mlabel):  # reordering testx and testy as [query_set, database] with equal number of queries per class.
        ...
        if train == -1:
            ...
            # cols=net.fc7.shape[-1]
            cols=net.fc7.get_shape().as_list()[-1]
            cols=int(cols)
            fc7=np.zeros((N,cols))
            ...
            for step in range(steps):
                ...
                fc7_1=sess.run(net.fc7,{x: batch_x, dropout: 1.})
                ...
                fc7[step * batch_size:(step + 1) * batch_size]=fc7_1
            ...
            encoding_2=np.asarray(np.cumsum(encoding_1, -1)[:,-1]>=int(m*0.4),np.uint8) # [N-1,]
            encoding_2_1 = encoding_2

            # 精搜索
            # 取编码层的上一层做欧式距离
            # fc8=fc7[1:N]*np.expand_dims(encoding_2,-1) # [n-1,512]

            fc8 = fc7[1:N][np.expand_dims(encoding_2, -1)] # 筛选满足粗搜索
            # fc8与fc7[0]做欧式距离，fc8为0的一行跳过

            # print(fc8.shape,type(fc8))
            # print(fc7[0].shape,type(fc7[0]))
            error=np.squeeze(fc8,1)-fc7[0]
            error=error**2

            Eu_dis=np.sum(error,1)/cols
            # Eu_dis=np.sum((np.squeeze(fc8,1)-fc7[0])**2,1)#/cols
            Eu_dis=np.asarray((Eu_dis < 0.001), np.uint8) # [x,]
            ...
            encoding_2 = encoding_3 * labels_1  # 编码一样，还有保证对应的真实标签是一样的（否则说明编码不对）

            comp1_cls_test = np.zeros((N - 1, 5))
            ...
            comp1_cls_test = pd.DataFrame(comp1_cls_test, columns=['score', 'encoding', 'True encoding', 'True label','Eu_dis'])
