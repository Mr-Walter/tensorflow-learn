...
class Net():
    ...
    def LabNet(self):
        ...
        net=tf.layers.dense(net,4096,activation=tf.nn.relu,trainable=self.trainable2,reuse=self.reuse,name='labnet_fc1')
        net=tf.layers.dense(net,512,activation=tf.nn.relu,trainable=self.trainable2,reuse=self.reuse,name='labnet_fc2')

        self.LabNet_fc = net
        net = tf.layers.dense(net, m, activation=None,trainable=self.trainable2,name='labnet_hash')
        ...
        # ----------重构label-------------------
        net=tf.nn.relu(net)
        # net=self.LabNet_label
        net=tf.layers.dense(net,512,activation=tf.nn.relu,trainable=self.trainable2,name='labnet_fc3')
        net = tf.layers.dense(net, 4096, activation=tf.nn.relu, trainable=self.trainable2,name='labnet_fc4')
        net = tf.layers.dense(net, num_classes, activation=tf.nn.softmax, trainable=self.trainable2,name='output')
...
if __name__ == "__main__":
    ...
    cost5=m-tf.reduce_sum(tf.cast(tf.equal(LabNet_label_2,ImgNet_label_2),tf.float32))/batch_size


    cost2=tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=y, logits=net.LabNet_new_label))
    cost4 = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=y, logits=ImgNet_pred))
    # -----------------------
    ...
    cost7 = tf.losses.mean_squared_error(net1, net2) # cost7 越大，  1/cost7 越小

    # -------------------------

    # cost = cost1 + cost3+cost5+cost6-cost7*10 +cost4
    # cost = cost1 + cost3 + cost5 + cost6 +10./cost7+ cost4
    cost=cost1+cost5+cost3+cost4*5+cost2*2
        ...
        if train == 1:
            for epoch in range(epochs):
                ...
                for step in range(steps):
                    ...
                    if step % 20 == 0:
                        acc,acc2=sess.run([accuracy,accuracy2],feed_dict={x: batch_x, y: batch_y, dropout: keep_rata})
                        print('epoch', epoch, '|', 'step:', step, '|', 'loss:', c,'|','acc',acc,'|','acc2',acc2)

                        # cost_1,cost_2,cost_3,cost_4,cost_5=sess.run([cost1,cost2,cost3,cost4,cost5],feed_dict={x: batch_x, y: batch_y, dropout: keep_rata})
                        # print('cost_1',cost_1,'cost_2',cost_2,'cost_3',cost_3,'cost_4',cost_4,'cost_5',cost_5)
                        # exit(-1)
