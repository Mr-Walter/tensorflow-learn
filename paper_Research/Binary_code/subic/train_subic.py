...
# Prepare data and model
  ...
  net = VGG_M_128()
  INPUT_SHAPE = (bsize,) + net.IMAGE_SHAPE # (batch_size,224,224,3)
  input_pl = tf.placeholder(tf.float32, shape= INPUT_SHAPE) # (batch_size,224,224,3)
  labels_pl = tf.placeholder(tf.int32, (bsize)) # (batch_size,)

  # 分类
  nclass = args.nclass  # 1000(imageNet)
  c_subic = CLASSIFY_SUBIC(net, nclass,  m, k) #

  logits = c_subic.inference(input_pl) # [batch_size,1000]
  sb_features = c_subic.sb_features # [batch_size,m*k]

# Entropy losses
  sb_codes = tf.split(sb_features, num_or_size_splits=m, axis=1) # m个 [batch_size,k] list
  sb_entropy = [None]*m
  sb_b = [None]*m
  sb_batch_ent = [None]*m
  batch_entropy = 0
  ...
  for i in range(m):
    ...
    sb_b[i] = finetune(tf.reduce_sum(sb_codes[i], 0)/tf.cast(tf.shape(sb_codes[i])[0], tf.float32)) + tf.constant(1e-30, dtype=tf.float32)
...
# Loss
  ...
  loss =  loss_cl + e_wt*ind_entropy - be_wt*batch_entropy  # 总loss
