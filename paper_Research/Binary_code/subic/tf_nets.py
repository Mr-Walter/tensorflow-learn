 ...
 def set_params(self, net_params, nlayers, sess, scope='', id_w='weights', id_b='biases'):
   with tf.variable_scope(scope, reuse=True):
     for i in range(self.nlayers):
       if(i<nlayers):
          sess.run(tf.get_variable(self.l_names[i]+'w').assign(net_params[self.l_names[i]][id_w])) # 赋值
...
class VGG_M_128(NET):
  def __init__(self):
    ...
    self.conv_ksize = [[7, 7, 3, 96], [5, 5, 96, 256], [3, 3, 256, 512], [3, 3, 512, 512], [3, 3, 512, 512]] # 5 ceng juanji
    self.weights = {}
    self.bias = {}

    # 5个卷积层
    for i in range(len(self.conv_ksize)): # 5
  ...
  def inference (self, images):
    pad='SAME'
    self.images = images # [1,224,224,3]
  # layer 1
    self.relu1 = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(images, self.weights[self.l_names[0]], strides=[1,2,2,1], padding='VALID'), self.bias[self.l_names[0]]))  # [1,109,109,96]
    ...
    self.pool1 = tf.nn.max_pool(self.lrn1, ksize=[1,3,3,1], strides=[1,2,2,1], padding='VALID', name='pool1') # [1,54,54,96]
  # layer 2
    self.relu2 = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(self.pool1, self.weights[self.l_names[1]], strides=[1,2,2,1], padding=pad), self.bias[self.l_names[1]])) # [1,27,27,256]
    radius = 2; alpha = 1e-04; beta = 0.75; bias = 1.0
    self.lrn2 = tf.nn.local_response_normalization(self.relu2, depth_radius=radius, alpha=alpha, beta=beta, bias=bias)
    self.pool2 = tf.nn.max_pool(self.lrn2, ksize=[1,3,3,1], strides=[1,2,2,1], padding=pad, name='pool2') # [1,14,14,256]
  # layer 3-4-5
    self.relu3 = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(self.pool2, self.weights[self.l_names[2]], strides=[1,1,1,1], padding=pad), self.bias[self.l_names[2]])) # [1,14,14,512]
    self.relu4 = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(self.relu3, self.weights[self.l_names[3]], strides=[1,1,1,1], padding=pad), self.bias[self.l_names[3]])) # [1,14,14,512]
    self.relu5 = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(self.relu4, self.weights[self.l_names[4]], strides=[1,1,1,1], padding=pad), self.bias[self.l_names[4]])) # [1,14,14,512]
    self.pool5 = tf.nn.max_pool(self.relu5, ksize=[1,3,3,1], strides=[1,2,2,1], padding='VALID', name='pool5') # [1,6,6,512]
  # fully connected
    pool5_flat = tf.reshape(self.pool5, [-1, np.int(np.prod(self.pool5.get_shape()[1:]))]) # [1,6*6*512] # prod 累积
    self.fc6 = tf.nn.relu(tf.matmul(pool5_flat, self.weights[self.l_names[5]]) + self.bias[self.l_names[5]]) # [1,4096]
    self.fc7 = tf.nn.relu(tf.matmul(self.fc6, self.weights[self.l_names[6]]) + self.bias[self.l_names[6]]) # [1,128]
    return self.fc7
  def set_params(self, net_params, nlayers, sess, scope='', id_w='weights', id_b='biases'):
    with tf.variable_scope(scope, reuse=True):
      for i in range(self.nlayers):
...
class SUBIC(NET):
  def __init__(self, net, m, k, no_softmax=False):
    ...
    self.l_names = ['ip']
    self.weights = { 'ipw': tf.get_variable('ipw',  [self.NET.output_size, self.output_size], initializer= tf.random_normal_initializer(stddev=0.1))} # [128,128]
  ...
  def inference(self, input_data, no_softmax=False):
    if(len(input_data.get_shape().as_list())==4):
      self.features = self.NET.inference(input_data) # [batch_size,128]
    else:
...
class CLASSIFY_SUBIC(NET):
  def __init__(self, net, nclass, m, k):
    ...
    self.m = m
    self.k = k
    self.nlayers=self.NET.nlayers+2 # 9
    self.output_size = m*k
    self.l_names = ['ip', 'cip']
    self.weights = {}
    self.bias = {}
    self.weights['ipw'] = tf.get_variable('ipw',  [self.NET.output_size, self.output_size], initializer= tf.random_normal_initializer(stddev=0.1)) # [128,m*k]
    self.bias['ipb'] = tf.get_variable('ipb',  [self.output_size], initializer=tf.constant_initializer(0.0))
    self.weights['cipw'] = tf.get_variable('cipw',  [self.output_size, self.Nclass], initializer= tf.random_normal_initializer(stddev=0.1)) # [m*k,1000]
    self.bias['cipb'] = tf.get_variable('cipb',  [self.Nclass], initializer=tf.constant_initializer(0.0))
  def inference(self, input_data):
    if(len(input_data.get_shape().as_list())==4):
      self.features = self.NET.inference(input_data) # [batch_size,128]
    else:
      self.features = input_data # [224,224,3]
    sb_features = tf.nn.relu(tf.matmul(self.features, self.weights['ipw']) + self.bias['ipb']) # [batch_size,m*k]
    feat = tf.split(sb_features, num_or_size_splits=self.m, axis=1) # m个 [batch_size,k] list
    out = [None]*self.m
    for i in range(self.m):
      out[i] = tf.nn.softmax(feat[i]) # 实现了按块加softmax
    self.sb_features = tf.concat(out, axis=1) # 拼接 [batch_size,m*k]
    self.scores = tf.matmul(self.sb_features, self.weights['cipw']) + self.bias['cipb'] # [batch_size,1000]
    return self.scores
  def set_params(self, net_params, nlayers, sess, scope='', id_w='weights', id_b='biases'):
    print('%d out of %d layers' %(nlayers, self.nlayers))
    self.NET.set_params(net_params, nlayers, sess, scope, id_w, id_b)
